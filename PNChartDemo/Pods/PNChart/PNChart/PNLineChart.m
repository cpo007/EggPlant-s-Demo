...
//    [self touchKeyPoint:touches withEvent:event];
//}
-(void)ViewDidScroll:(CGPoint)point {
//    NSLog(@"%@",self.pathPoints);
//    [self.CGPointArray enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
//        CGFloat pointY = CGPointFromString(obj).y;
//        
//        NSLog(@"%f",pointY);
//    }];
...
//            if (distance <= 5.0) {
                ...
                for (UIBezierPath *path in _chartPath) {
//                    NSLog(@"%@",path);
//                    NSLog(@"%ld",[self.chartPath indexOfObject:path]);
                    BOOL pointContainsPath = CGPathContainsPoint(path.CGPath, NULL, p1, NO);
                    if (pointContainsPath) {
                        [_delegate userClickedOnLinePoint:touchPoint lineIndex:[_chartPath indexOfObject:path]];
...
- (void)calculateChartPath:(NSMutableArray *)chartPath andPointsPath:(NSMutableArray *)pointsPath andPathKeyPoints:(NSMutableArray *)pathPoints andPathStartEndPoints:(NSMutableArray *)pointsOfPath
{
    ...
    for (NSUInteger lineIndex = 0; lineIndex < self.chartData.count; lineIndex++) {
        ...
        [pathPoints enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            [obj enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                NSValue *p = obj;
                
                CGPoint point = [p CGPointValue];
                [self.CGPointArray addObject:NSStringFromCGPoint(point)];
...
- (void)setupDefaultValues
{
    ...
    _CGPointArray        = [[NSMutableArray alloc] init];
